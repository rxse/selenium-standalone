...
function computeFsPaths(opts) {
  ...
  if (opts.drivers.opera) {
    fsPaths.opera = {
      installPath: path.join(opts.basePath, 'operadriver', opts.drivers.opera.version + '-' + opts.drivers.opera.arch + '-operadriver')
    };
    if(process.platform === 'win32'){
      fsPaths.opera.installPath = fsPaths.opera.installPath + ".exe";
    }
  }
