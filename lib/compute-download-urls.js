...
var urls = {
  ...
  firefox: '%s/%s/%s-%s-%s',
  opera: '%s/v.%s/operadriver_%s.zip'
};
...
function computeDownloadUrls(opts, askedOpts) {
  ...
  if (opts.drivers.edge) {
    var url = getEdgeDriverUrl(opts.drivers.edge.version);
    if (url) {
      downloadUrls.edge = url;
    }
  }
  if (opts.drivers.opera) {
    downloadUrls.opera = util.format(
      urls.opera,
      opts.drivers.opera.baseURL,
      opts.drivers.opera.version,
      getOperaDriverArchitecture(opts.drivers.opera.version, opts.drivers.opera.arch)
    );
  }
  return downloadUrls;
}
...
function getOperaDriverArchitecture(version, wantedArchitecture) {
  switch (process.platform) {
    case 'linux':
      return getLinuxOperaDriverArchitecture(version, wantedArchitecture);
    case 'darwin':
      return getMacOperaDriverArchitecture(version, wantedArchitecture);
    case 'win32':
      return getWindowsOperaDriverArchitecture(version, wantedArchitecture);
    default:
      throw new Error('No Opera driver is available for platform "' + process.platform + '"');
  }
}

function getLinuxOperaDriverArchitecture(version, wantedArchitecture) {
  // Since 2.29, operadriver support was dropped for linux 32
  if (wantedArchitecture !== 'x64') {
    throw new Error('Only x64 architecture is available for operadriver on linux')
  }

  if (compareVersionsOpera(version, '2.27') < 0) {
    throw new Error('Only versions >= 2.27 are available for operadriver on linux')
  }

  return 'linux64';
}

function getMacOperaDriverArchitecture(version, wantedArchitecture) {
  // Since 2.29, operadriver support was dropped for mac 32
  if (wantedArchitecture !== 'x64') {
    throw new Error('Only x64 architecture is available for operadriver on mac')
  }

  if (compareVersionsOpera(version, '2.27') < 0) {
    throw new Error('Only versions >= 2.27 are available for operadriver on mac')
  }

  return 'mac64';
}

function getWindowsOperaDriverArchitecture(version, wantedArchitecture) {
  if (compareVersionsOpera(version, '2.27') < 0) {
    throw new Error('Only versions >= 2.27 are available for operadriver on mac')
  }
  return 'win' + (wantedArchitecture === 'x64' ? '64' : '32');
}
 ...
 */
function compareVersionsOpera(v1, v2) {
  ...
  var v1Numbers = v1parts.map(function (v) {
    return parseInt(v);
  });
  var v2Numbers = v2parts.map(function (v) {
    return parseInt(v);
  });
