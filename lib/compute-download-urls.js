...
function getOperaDriverArchitecture(version, wantedArchitecture) {
  switch (process.platform) {
    ...
    case 'darwin' :
      return getMacOperaDriverArchitecture(version);
    case 'win32' :
...
function getLinuxOperaDriverArchitecture(version, wantedArchitecture) {
  ...
  if(compareVersions(version, '2.37') >= 0 && wantedArchitecture !== 'x64') {
    throw new Error('Only x64 architecture is available for operadriver >= 2.37')
  }
