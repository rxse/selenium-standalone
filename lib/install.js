...
function install(opts, cb) {
  ...
  if (fsPaths.opera) {
    tasks.push(setDriverFilePermissions.bind(null, fsPaths.opera.installPath));
  }
  ...
  function download(opts, cb) {
    ...
    if (opts.fsPaths.opera) {
      installers.push({
        installer: installOperaDr,
        from: opts.urls.opera,
        to: opts.fsPaths.opera.downloadPath
      })
    }
  ...
  function installOperaDr(opts, cb) {
    getDownloadStream(opts.from, function (err, stream) {
      ...
      var zipWriteStream = fs.createWriteStream(opts.to)
        .once('error', cb.bind(null, new Error('Could not write to ' + opts.to)));
      stream.pipe(zipWriteStream);

      // Uncompress downloaded file
      zipWriteStream.once('finish',
        uncompressOperaDownloadedFile.bind(null, opts.to, cb)
      );
    });
  }

  function uncompressOperaDownloadedFile(zipFilePath, cb) {
    ...
    yauzl.open(zipFilePath, { lazyEntries: true }, function onOpenZipFile(err, zipFile) {
      ...
      zipFile.on('entry', function (entry) {
        if (/\/$/.test(entry.fileName)) {
          ...
          if (entry.fileName.startsWith('operadriver')) {
...
function logInstallSummary(logger, paths, urls) {
  ['selenium', 'chrome', 'ie', 'firefox', 'edge', 'opera'].forEach(function log(name) {
    if (!paths[name]) {
      return;
    }
